...
# 定义输出文件路径
output_path = './data/万里牛数据分类后.xlsx'
        ...
        "query": {
            "bool": {
                "must": [
                    ...
                    {"term": {"order_time.keyword": buyer_times}} if buyer_times else {"match_all": {}}
                ]
            }
        }
    }
    response = es.search(index=index, body=query)
    return response['hits']['total']['value'] > 0  # 返回是否已存在记录
...
# 加载 Excel 数据
updated_file_path = './data/万里牛数据分类后.xlsx'
...
if not es.indices.exists(index='orders'):
    es.indices.create(index='orders', body={"mappings": index_mappings})

# 准备数据并上传到 Elasticsearch
actions = []
for index, row in updated_df.iterrows():
    source_data = {col: row[col] for col in updated_df.columns}
    # 使用 record_exists 检查记录是否存在，若不存在则加入上传队列
    if not record_exists(es, 'orders', row['buyer_id'], row['online_product_id'], row.get('order_time')):
        action = {
            "_index": "orders",
            "_source": source_data
        }
